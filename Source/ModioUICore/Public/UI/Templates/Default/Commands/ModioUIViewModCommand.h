...
#include "ModioUIViewModCommand.generated.h"

/**
 *
 */
UCLASS()
class MODIOUICORE_API UModioUIViewModCommand: public UObject, public IModioUICommandObject
{
	GENERATED_BODY()
protected:
	static inline const FString CommandID {TEXT("ViewMod")};

	UPROPERTY(Transient, BlueprintReadOnly, Category = "mod.io|UI|Commands|View Mod", meta = (BlueprintProtected))
	UObject* DataSource;
	...
	{
		bool bUnused;
		return UModioUICommandLibrary::GetCommandText(CommandID, "ViewModAction", bUnused);
	}

	virtual FSlateBrush GetCommandBrush_Implementation() override
	{
		bool bUnused;
		return UModioUICommandLibrary::GetCommandBrush(CommandID, "ViewModAction", bUnused);
	}
	...
	virtual void Execute_Implementation(UObject* Context) override
	{
		if (UModioUISubsystem* UISubsystem = GEngine->GetEngineSubsystem<UModioUISubsystem>())
		{
			...
			{
				UISubsystem->RequestShowDialog(EModioUIDialogType::ModDetails, DataSource);
			}
		}
	}
