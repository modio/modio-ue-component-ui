...
#include "ModioDefaultTokenPackTile.generated.h"

/**
 * 
 */
UCLASS()
class MODIOUICORE_API UModioDefaultTokenPackTile : public UModioUIComponentBase,
...
protected:
	...
	UFUNCTION(BlueprintCallable, Category = "mod.io|UI|Events|Token Pack Tile", meta = (BlueprintProtected))
	void NotifyClicked();

	UPROPERTY(BlueprintReadOnly, VisibleAnywhere, Category = "mod.io|UI|Token Pack Tile", meta = (BlueprintProtected))
	...
	/**
	 * @brief Handles both getting the selected state from an associated list if there is one, and falls back on the
	 * local state variable otherwise
	 */
	UFUNCTION(BlueprintCallable, Category = "mod.io|UI|Token Pack Tile", meta = (BlueprintProtected))
	...
	UPROPERTY(BlueprintReadOnly, EditAnywhere, Category = "mod.io|UI|Data Binding|Token Pack Tile",
