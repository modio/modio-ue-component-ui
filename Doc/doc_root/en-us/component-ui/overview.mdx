...
:::warning[Register a portal interface]
Component UI routes platform-specific requests such as SSO auth tokens, entitlements, token packs, and the platform storefront through a portal interface you provide.  Register one with `UModioUISubsystem::RegisterPortalInterface`, passing an object that implements `IModioPortalInterface`.

If no portal interface is registered, these calls will fail and log an error to the `ModioUICore` category.
:::
